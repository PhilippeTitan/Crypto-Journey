  ...
  return (
    <div className="relative w-full max-w-3xl mx-auto z-30">
      ...
      {configOpen && (
        <div
          ref={popoverRef}
          className="absolute bottom-full mb-3 left-0 w-full sm:w-[420px] bg-[#090b0f]/95 border border-zinc-700/40 rounded-xl shadow-2xl backdrop-blur-xl p-4 text-xs font-mono text-zinc-300 z-50 animate-in fade-in slide-in-from-bottom-2 duration-200"
        >
          <div className="flex items-center justify-between pb-3 mb-3 border-b border-zinc-800">
            <div className="flex items-center gap-2">
              <span className="text-cyan-400 font-semibold tracking-wider text-[11px]">AGENT CONFIGURATION</span>
          ...
          {activeTab === 'main' && (
            <div className="space-y-3">
              ...
              <div
                ...
                className="flex items-center justify-between p-2.5 rounded-lg bg-zinc-900/60 border border-zinc-800/80 hover:border-cyan-500/40 cursor-pointer transition-colors"
              >
                <div>
                  <div className="text-[10px] text-zinc-500 uppercase tracking-wider">Inference Provider</div>
                  <div className="text-zinc-200 font-medium text-xs mt-0.5 flex items-center gap-1.5">
              ...
              >
                <div>
                  <div className="text-[10px] text-zinc-500 uppercase tracking-wider">Reasoning Effort</div>
                  <div className="text-zinc-200 font-medium text-xs mt-0.5 flex items-center gap-2">
                    <span className={`inline-block w-2 h-2 rounded-full ${
                      effort === 'Extra High' ? 'bg-purple-400' : effort === 'High' ? 'bg-cyan-400' : 'bg-emerald-400'
                    }`} />
                    <span>{effort}</span>
                    <span className="text-[10px] text-zinc-500">(Decoupled from Autonomy)</span>
                  </div>
                </div>
                <span className="text-zinc-500 text-xs">›</span>
              </div>
              ...
              >
                <div>
                  <div className="text-[10px] text-zinc-500 uppercase tracking-wider">Structured Context</div>
                  <div className="text-zinc-200 text-xs mt-0.5 truncate max-w-[280px]">
              ...
              <div className="grid grid-cols-2 gap-2 pt-1">
                <div className="p-2 rounded bg-zinc-900/40 border border-zinc-800 text-[10px]">
                  <span className="text-zinc-500 block">Capabilities</span>
                  <span className="text-cyan-300 font-semibold text-xs">18 Registered</span>
                </div>
                <div className="p-2 rounded bg-zinc-900/40 border border-zinc-800 text-[10px]">
                  <span className="text-zinc-500 block">Skills Layer</span>
                  <span className="text-emerald-300 font-semibold text-xs">8 Available</span>
                </div>
              </div>

              {/* Autonomy Regime */}
              <div className="pt-2 border-t border-zinc-800">
                <div className="text-[10px] text-zinc-500 uppercase tracking-wider mb-1.5">Autonomy Regime</div>
                <div className="grid grid-cols-3 gap-1.5">
                  {(['soft', 'normal', 'aggressive', 'protect', 'preserve', 'emergency'] as const).map((m) => (
                    <button
                      key={m}
                      onClick={() => onSelectAutonomy(m)}
                      className={`px-2 py-1.5 rounded text-[10px] font-mono capitalize transition-colors ${
          ...
          {activeTab === 'provider' && (
            <div className="space-y-3">
              <button
                ...
                className="text-zinc-500 hover:text-zinc-300 text-[10px] flex items-center gap-1 mb-2"
              >
                ‹ Back to Agent Config
              </button>
              <div className="text-[10px] text-zinc-500 uppercase tracking-wider">Select AI Provider</div>
                  ...
                  <div
                    ...
                    className={`p-2 rounded-lg cursor-pointer flex items-center justify-between ${
                      selectedProvider === p.key
                        ...
                        : 'bg-zinc-900/40 text-zinc-400 hover:bg-zinc-900/80 hover:text-zinc-200 border border-transparent'
                    }`}
                  >
                    <span>{p.name}</span>
                    <span className="text-[9px] text-zinc-500">{p.models.length} models</span>
                  </div>
                ))}
              </div>
              ...
              {currentProviderObj && (
                <div className="pt-2 border-t border-zinc-800">
                  <div className="text-[10px] text-zinc-500 uppercase tracking-wider mb-1">Available Models</div>
                  <div className="space-y-1">
                    {currentProviderObj.models.map((m) => (
                      <div
                        key={m}
                        onClick={() => setSelectedModel(m)}
                        className={`p-2 rounded text-[11px] cursor-pointer ${
                          selectedModel === m
          ...
          {activeTab === 'effort' && (
            <div className="space-y-3">
              ...
              <div className="text-[10px] text-zinc-500 uppercase tracking-wider">Reasoning Effort Tier</div>
              {[
                ...
                >
                  <div className="font-medium text-xs text-zinc-200">{tier.name}</div>
          ...
          {activeTab === 'context' && (
            <div className="space-y-3">
              ...
              <div className="text-[10px] text-zinc-500 uppercase tracking-wider">Structured Context Bundle</div>
              {Object.entries(contextToggles).map(([key, val]) => (
                <label
                  key={key}
                  className="flex items-center justify-between p-2 rounded-lg bg-zinc-900/40 border border-zinc-800 cursor-pointer text-xs"
                >
      ...
      <form
        ...
        className={`relative flex flex-col bg-[#07090d]/90 border rounded-xl backdrop-blur-xl transition-all duration-300 ${
          focused
      ...
      >
        {/* Top Input Row */}
        <div className="flex items-center px-3.5 pt-2.5 pb-1">
          <span className="text-cyan-400/80 font-mono text-xs select-none mr-2">›</span>
          <input
            ref={inputRef}
            type="text"
            value={input}
            ...
            placeholder={
              running
                ...
                : 'Ask MaurEdge or type "scan market", "12 - 10000", "why DOGE?"...'
            }
            className="flex-1 bg-transparent border-0 outline-none text-zinc-200 placeholder-zinc-600 font-mono text-xs"
          />
          <button
            type="submit"
            disabled={!input.trim()}
        ...
        <div className="flex items-center justify-between px-3 pb-2 pt-1 text-[10px] font-mono text-zinc-500 border-t border-zinc-800/40">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <button
              ...
              className={`px-1.5 py-0.5 rounded transition-colors flex items-center gap-1 ${
                configOpen
                  ...
                  : 'bg-zinc-900/60 hover:bg-zinc-800/80 text-zinc-400 border border-zinc-800'
              }`}
            >
              <span>＋</span>
              <span className="hidden sm:inline text-[9px]">Config</span>
            </button>
            ...
            >
              <strong className="text-zinc-400 font-normal">{currentProviderObj?.name || 'Gemini'}</strong>{' '}
            ...
            <span
              ...
              className="cursor-pointer hover:text-zinc-300 transition-colors"
            >
              Effort: <span className="text-zinc-300">{effort}</span>
            </span>
            ...
            <span
              ...
              className="cursor-pointer hover:text-zinc-300 transition-colors uppercase text-[9px]"
            >
          ...
          {/* Running Indicator */}
          {running && (
            <div className="flex items-center gap-1.5 text-cyan-400 text-[9px] animate-pulse">
