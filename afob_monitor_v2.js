...
const ENTRY_PRICE = 0.0003186;
const ENTRY_VALUE = 10.58;
const TOKENS_HELD = 33205.14;
const TAKE_PROFIT = 1.06;   // +10% = +$1.06
const STOP_LOSS = 0.53;     // -5% = -$0.53
...
async function monitor() {
  console.log('💎 AFOB LIVE MONITOR v2');
  console.log('========================');
  ...
  while (true) {
    try {
      ...
      const emoji = pnl >= 0 ? '🟢' : '🔴';
      console.log(`[${time}] ${emoji} $${currentPrice.toFixed(8)} | $${currentValue.toFixed(2)} | PnL: ${pnl >= 0 ? '+' : ''}$${pnl.toFixed(2)} (${pnl >= 0 ? '+' : ''}${pnlPct}%) | 5m: ${data.m5 > 0 ? '+' : ''}${data.m5}% | ${data.buys5m}B/${data.sells5m}S [${pressure}]`);
      ...
      if (pnl >= TAKE_PROFIT) {
        ...
        try {
          const qty = (TOKENS_HELD - 1).toFixed(2);
          const result = execSync(`baw market-order swap --fromTokenQty ${qty} --fromToken ${AFOB_ADDRESS} --toToken 0x55d398326f99059fF775485246999027B3197955 --binanceChainId 56 --slippage 3 --mev true --gasLevel LOW --json`, { encoding: 'utf8', timeout: 30000 });
      ...
      // TRAILING STOP — if we hit +5% but then drop 2% from peak
      if (highWaterMark > ENTRY_VALUE * 1.05) {
        const trailingStop = highWaterMark * 0.98;
